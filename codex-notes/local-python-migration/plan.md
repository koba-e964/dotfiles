...
## Files to change
- `install-local-python.sh` (renamed from `tentative-install-python.sh`)
- `install.sh`
...
1. Add `python/requirements-local_python.txt` with pinned packages currently installed in the local Python install script.
...
2. Rename `tentative-install-python.sh` to `install-local-python.sh` and make it an idempotent manager script:
...
3. Call `./install-local-python.sh` from `install.sh` so local Python setup is part of standard dotfiles bootstrap.
...
## Test strategy
- Run shell syntax check for changed scripts (`sh -n install.sh install-local-python.sh`).
- Execute `./install-local-python.sh` once and confirm:
...
- [x] Add tracked requirements file for `~/local_python` packages.
- [x] Add hashes to locked requirements and enforce `--require-hashes`.
- [x] Rename installer to `install-local-python.sh` and make it idempotent and requirements-driven.
- [x] Invoke local Python sync in `install.sh`.
- [x] Guard zsh activation to avoid startup failures on fresh machines.
- [x] Update `todo.txt` to reflect completion.
