...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。**実行者自身が今このターンで呼べる exact な subagent dispatch 手段**（例: `Task tool` / `spawn_agent` / `Agent` tool）で **新規 subagent を dispatch** する。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。`利用可能な dispatch 手段` 節に API 名が列挙されている場合は、その中から **実際に呼べるものを 1 つ選んで使う**。列挙が `none` なら empirical 実行は始めず、`dispatch unavailable` に進む。並列で複数シナリオを同時実行する場合は単一メッセージ内で複数 Agent 呼び出しを並べる。dispatch 不能環境の扱いは「環境制約」節を参照。
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - ステップ数（dispatch 手段の戻り値に付く usage メタの `tool_uses` をそのまま使う。Read / Grep も含める、除外しない）
     - 所要時間（dispatch 手段の戻り値に付く usage メタの `duration_ms`）
...
## 利用可能な dispatch 手段
<この実行者自身が今このターンで呼べる exact な API / tool 名を列挙。例: `spawn_agent`。親セッションだけが使える手段は書かない。1 つも無い / 禁止 / 未確認なら `none` と書く>
...
呼び出し側はレポートから自己申告部分を抽出し、`tool_uses` / `duration_ms` を **その環境の subagent dispatch 手段** の usage メタから取得して評価軸表を埋める。

## 環境制約

新規 subagent を dispatch できない環境（既に subagent として動作している、`Task tool` / `spawn_agent` / `Agent` tool などの dispatch 手段が存在しない、または無効化されている等）では、本 skill は **適用しない**。
- **skip 判定の前に**、「名前が違うだけで等価な subagent dispatch 手段がないか」を確認する。`Task tool が無い` だけでは skip しない。
- **子孫 dispatch が禁止された subagent** も `dispatch unavailable` 扱いにする。親セッションで使えた手段を、自分も使えると仮定しない。
- `dispatch unavailable` は次のいずれかで成立する: `利用可能な dispatch 手段 = none`、列挙された API 名をこの実行者が実際には呼べない、または子孫 dispatch が禁止されている。
- `dispatch unavailable` が成立したら、**empirical 評価の経路はそこで停止** する。以後は empirical の成功/失敗や収束判定を進めない。
...
**構造審査モード**: empirical 評価とは**別経路の静的レビュー**として扱う。skill / プロンプトの **記述の整合性・明瞭性だけ** をチェックしたい場合は、構造審査モードとして明示的に切り分ける。`dispatch unavailable` でも、親セッションが直接レビューするか、利用可能なら別の fresh subagent に「今回は構造審査モード: 実行ではなくテキスト整合性チェック」と明記して依頼してよい。構造審査は empirical の代替ではなく補助であり、連続クリア判定・成功/失敗・収束判定には使わない。
